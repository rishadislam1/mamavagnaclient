...
import { MonthBaseBuyerListApi } from "../../apiRequest/MonthBaseBuyerListApi";
import Swal from "sweetalert2";
import { BASE_URL } from "../../../public/config";

const MonthBasisBuyerList = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      const buyerData = await MonthBaseBuyerListApi(month,year);
      ...
      if (result.isConfirmed) {
        ...
        fetch(url, {
          ...
          .then(() => {
            ...
            const updatedBuyers = buyerData.filter((buyer) => buyer.id !== id);
            setBuyerData(updatedBuyers);
            ...
            {filteredBuyers.length > 0 &&
              filteredBuyers?.map((buyer, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    {buyer?.shares?.length > 0 ? (
                      <div>
                        {buyer.shares.map((share) => (
                          <div key={share.id}>
                            <b>Name:</b> {share.name}, <b>Shareable Amount:</b>{" "}
                            {share.percentage} tk <br />
                            ---------
                          </div>
                        ))}
                      </div>
...
export default MonthBasisBuyerList;
